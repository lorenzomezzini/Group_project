...
```{r, fig.cap = "Raw data - Estimated Crimes 1979-2019 in US", echo = FALSE, message = FALSE, warning=FALSE}
 DT::datatable(estimated_crimes_1979_2019, options = list( scrollX='400px', scrollY='300px') )
```
...
From above we can see how many NAs we have for each feature. Looking at this we already decide to not take into account <b>rape_revised</b> and <b>caveats</b>, while we already know that <b>state_name</b> and <b>state_abbr</b> missing values refers to United States, so we will fill them appropriately.
...
```{r, fig.cap= 'Cleaned data - Estimated Crimes 2004-2013 in US', echo = FALSE, message = FALSE, warning=FALSE}
  DT::datatable(estimated_crimes, options = list(scrollX='400px', scrollY='300px'))
```
